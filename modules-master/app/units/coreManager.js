...
class CoreManager {
    ...
    initSync() {
        setInterval(() => {
            ...
            if (currentState === 5 && this.lastState !== 5) {
                console.log("🏁 [KrooveCore] 检测到播放结束，自动切歌...");
                this.next(true); // 传入 true 代表这是内部引擎跑完触发的自动切歌
            }
    ...
    async bootstrap() {
        ...
        const playlistCount = dbManager.db.prepare('SELECT count(*) as count FROM tracks').get();
    ...
    handleCommand(cmd) {
        switch(cmd.cmd) {
            case 'get_sync': this.syncCurrentState(); break; // 前端连接后主动请求一次同步
            case 'set_mode': playlist.setMode(cmd.mode); break; // 切换播放模式
            case 'play_prev': this.prev(); break;
            case 'play_next': this.next(); break;
            case 'play_by_id': this.playById(cmd.id); break;
            case 'pause': this.pause(); break;
            case 'play': this.resume(); break;
            case 'seek': this.engine.seek(cmd.seconds, cmd.relative || false); break;
            case 'set_volume': this.engine.setVolume(cmd.volume); break;
            case 'set_mute': this.engine.setMute(cmd.mute); break;
            case 'add_folder': this.addLibraryFolder(cmd.path); break;
            case 'remove_folder': this.removeLibraryFolder(cmd.path); break;
        }
    }
}
